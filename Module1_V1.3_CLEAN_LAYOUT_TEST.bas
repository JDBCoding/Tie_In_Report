...
Sub TextFormat()
    ...
    End If
    Next i
    ' Pull descriptions and bullets closer to job numbers

    For i = 1 To lastRow

        ' Job description rows

        If dataCleanSheet.Cells(i, "B").Value <> "" And _

           dataCleanSheet.Cells(i, "D").Value <> "" And _

           Left(Trim(dataCleanSheet.Cells(i, "D").Value), 1) <> Chr(149) Then

            dataCleanSheet.Cells(i, "C").Value = dataCleanSheet.Cells(i, "D").Value

            dataCleanSheet.Cells(i, "D").ClearContents

            dataCleanSheet.Cells(i, "B").HorizontalAlignment = xlRight

            dataCleanSheet.Cells(i, "C").HorizontalAlignment = xlLeft

            dataCleanSheet.Cells(i, "C").IndentLevel = 0

        End If

        ' Bullet/comment rows

        If Left(Trim(dataCleanSheet.Cells(i, "D").Value), 1) = Chr(149) Then
    ...
    ' Job descriptions and comments

    With dataCleanSheet.Range("C1:C" & lastRow)
    ...
    ' Hide old description column after moving data into C

    dataCleanSheet.Columns("D").Hidden = False
...
Private Function TIR_BuildEmailHtml(ByVal ws As Worksheet, ByVal firstRow As Long, ByVal lastRow As Long) As String
    ...
    html = html & "<table cellpadding='0' cellspacing='0' style='border-collapse:collapse;border:none;width:1000px;font-family:Calibri,Arial,sans-serif;font-size:11pt;table-layout:fixed;'>"
    html = html & "<colgroup>"
    html = html & "<col style='width:110px;'>"
    html = html & "<col style='width:145px;'>"
    html = html & "<col style='width:745px;'>"
    html = html & "</colgroup>"
    ...
    For r = firstRow To lastRow
        ...
        If TIR_RowIsBlank(bText, cText, dText) Then
            html = html & "<tr><td colspan='3' style='border:none;height:8px;font-size:4pt;line-height:4pt;padding:0;'>&nbsp;</td></tr>"
        ElseIf TIR_IsLegendRow(bText, cText, dText) Then
            html = html & TIR_RenderFullWidthRow(ws, r, bText, cText, dText, "center", True)
        ElseIf TIR_IsSummaryRow(bText, cText, dText) Then
            html = html & TIR_RenderFullWidthRow(ws, r, bText, cText, dText, "center", True)
        ElseIf TIR_IsPomDateRow(bText, cText, dText) Then
            html = html & TIR_RenderFullWidthRow(ws, r, bText, cText, dText, "left", True)
        ...
        Else
            ...
            For c = 2 To 4
                ...
                rowHtml = rowHtml & TIR_RenderCell(cell)
            Next c
            rowHtml = rowHtml & "</tr>"
            html = html & rowHtml
        End If
