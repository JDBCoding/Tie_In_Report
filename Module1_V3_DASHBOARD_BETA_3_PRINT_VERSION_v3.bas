...
Sub OpenEmailTieIn()
    ...
    Dim printVersionPath As String
    ...
    printVersionPath = TIR_CreatePrintVersionPdf(ws, 4, lastRow)
    ...
    With outlookMail
        ...
        If Len(printVersionPath) > 0 Then .Attachments.Add printVersionPath
    End With

    If Len(printVersionPath) = 0 Then
        MsgBox "Email created, but the print version PDF could not be created.", vbExclamation
    End If
...
Private Function TIR_CreatePrintVersionPdf(ByVal ws As Worksheet, ByVal firstRow As Long, ByVal lastRow As Long) As String
    Dim printHtml As String
    ...
    On Error GoTo FailClean

    tempFolder = Environ$("TEMP")
    If Right$(tempFolder, 1) <> "\" Then tempFolder = tempFolder & "\"

    htmlPath = tempFolder & "BDS Field Tie In - Print Version.htm"
    pdfPath = tempFolder & "BDS Field Tie In - Print Version.pdf"

    printHtml = TIR_BuildPrintVersionHtml(ws, firstRow, lastRow)
    ...
    On Error Resume Next
    If Len(Dir$(pdfPath)) > 0 Then Kill pdfPath
    On Error GoTo FailClean

    Set wordApp = CreateObject("Word.Application")
    wordApp.Visible = False
    Set wordDoc = wordApp.Documents.Open(htmlPath, False, True)

    With wordDoc.PageSetup
        .Orientation = 0
        .TopMargin = wordApp.InchesToPoints(0.35)
        .BottomMargin = wordApp.InchesToPoints(0.35)
        .LeftMargin = wordApp.InchesToPoints(0.35)
        .RightMargin = wordApp.InchesToPoints(0.35)
    End With
    ...
    If Len(Dir$(pdfPath)) > 0 Then
        TIR_CreatePrintVersionPdf = pdfPath
    Else
        TIR_CreatePrintVersionPdf = ""
    End If
...
Private Function TIR_BuildPrintVersionHtml(ByVal ws As Worksheet, ByVal firstRow As Long, ByVal lastRow As Long) As String
    Dim html As String
    Dim r As Long
    Dim bText As String
    Dim cText As String
    ...
    Dim generatedLine As String
    Dim aircraftStarted As Long
    ...
    ' First pass: collect simple cover-page numbers.
    For r = firstRow To lastRow
        ...
        If TIR_IsPomDateRow(bText, cText, dText) Then
            generatedLine = TIR_CombineRowText(bText, cText, dText)
        ElseIf TIR_IsAirplaneHeaderRow(ws, r) Then
            aircraftCount = aircraftCount + 1
        ElseIf Not TIR_RowIsBlank(bText, cText, dText) _
            And Not TIR_IsLegendRow(bText, cText, dText) _
            And Not TIR_IsSummaryRow(bText, cText, dText) _
            And Not TIR_IsBulletRow(bText, cText, dText) _
            And Not (bText <> "" And cText = "" And dText = "" And InStr(1, bText, ":", vbTextCompare) = 0) Then
            totalJobs = totalJobs + 1
        End If
    Next r

    If Len(generatedLine) = 0 Then generatedLine = "Generated from current Tie-In data"

    html = ""
    html = html & "<html><head><meta charset='utf-8'>"
    html = html & "<style>"
    html = html & "@page { size: 8.5in 11in; margin: 0.35in; }"
    html = html & "body { margin:0; padding:0; background:#FFFFFF; font-family:Calibri,Arial,sans-serif; font-size:10.5pt; color:#202020; }"
    html = html & ".page { width:7.35in; margin:0 auto; }"
    html = html & ".coverTitle { text-align:center; font-size:22pt; font-weight:bold; letter-spacing:.4pt; margin:28px 0 4px 0; color:#202020; }"
    html = html & ".coverSub { text-align:center; font-size:15pt; font-weight:bold; letter-spacing:.3pt; margin:0 0 16px 0; color:#333333; }"
    html = html & ".infoBox { border:1px solid #BFBFBF; background:#F7F7F7; padding:10px 14px; margin:0 auto 18px auto; width:5.8in; font-size:11pt; line-height:16pt; }"
    html = html & ".rule { border-top:2px solid #333333; margin:10px 0 0 0; }"
    html = html & "</style></head>"
    html = html & "<body>"

    ' Cover / report header page.
    html = html & "<div class='page'>"
    html = html & "<div class='coverTitle'>BDS FIELD TIE-IN REPORT</div>"
    html = html & "<div class='coverSub'>FULL AIRCRAFT WORK LIST</div>"
    html = html & "<div class='infoBox'>"
    html = html & "<div><b>" & TIR_HtmlEncode(UCase$(generatedLine)) & "</b></div>"
    html = html & "<div>AIRCRAFT: <b>" & aircraftCount & "</b></div>"
    html = html & "<div>TOTAL JOB LINES: <b>" & totalJobs & "</b></div>"
    html = html & "</div>"
    html = html & "<div class='rule'></div>"
    html = html & "</div>"

    ' Full aircraft work list. Each aircraft starts on a fresh printed page.
    For r = firstRow To lastRow
        bText = TIR_CleanCellText(ws.Cells(r, "B").Value)
        cText = TIR_CleanCellText(ws.Cells(r, "C").Value)
        dText = TIR_CleanCellText(ws.Cells(r, "D").Value)
        ...
        If TIR_RowIsBlank(bText, cText, dText) Then
            ' Skip worksheet spacer rows.

        ElseIf TIR_IsLegendRow(bText, cText, dText) Or TIR_IsSummaryRow(bText, cText, dText) Or TIR_IsPomDateRow(bText, cText, dText) Then
            ' Skip dashboard-only rows in the print PDF.
        ...
        ElseIf TIR_IsAirplaneHeaderRow(ws, r) Then
            ...
            aircraftStarted = aircraftStarted + 1
            html = html & "<div class='page' style='page-break-before:always;'>"
            html = html & "<div style='border-top:3px solid #333333;border-bottom:3px solid #333333;padding:6px 0;margin:0 0 9px 0;text-align:center;font-weight:bold;font-size:15pt;line-height:18pt;color:#000000;'>" & TIR_HtmlEncode(UCase$(rowText)) & "</div>"
            aircraftOpen = True
        ...
        ElseIf TIR_IsBulletRow(bText, cText, dText) Or Left(cText, 1) = ChrW(8226) Or Left(dText, 1) = ChrW(8226) Then
            ...
            html = html & "<div style='margin-left:150px;margin-top:1px;margin-bottom:5px;color:" & commentColor & ";font-weight:bold;font-size:10.5pt;line-height:14pt;'>&bull;&nbsp;" & TIR_HtmlEncode(UCase$(jobText)) & "</div>"
        ...
        ElseIf bText <> "" And cText = "" And dText = "" And InStr(1, bText, ":", vbTextCompare) = 0 Then
            ...
            html = html & "<div style='margin:8px 12px 0 12px;padding:0 0 2px 0;border-top:1px solid #D9D9D9;'>"
            html = html & "<div style='display:inline-block;margin-top:-1px;padding:4px 8px 2px 0;color:" & categoryColor & ";font-weight:bold;text-decoration:underline;font-size:11.5pt;line-height:15pt;'>" & TIR_HtmlEncode(UCase$(currentCategory)) & "</div>"
            categoryOpen = True
        ...
        Else
            ...
            html = html & "<div style='margin-left:34px;margin-top:2px;margin-bottom:2px;line-height:15pt;font-size:10.5pt;color:" & descColor & ";font-weight:" & fontWeight & ";'>"
    ...
    TIR_BuildPrintVersionHtml = html
End Function


 ' ============================================================
' DASHBOARD EMAIL BUILDER v3.2 BETA
' Purpose:
...
Private Function TIR_BuildDashboardEmailHtml(ByVal ws As Worksheet, ByVal firstRow As Long, ByVal lastRow As Long) As String
    ...
    html = html & "<div style='width:1320px;margin:0 auto;font-family:Calibri,Arial,sans-serif;font-size:12pt;color:#222222;'>"
...
Private Function TIR_DashExecutiveSummary(ByVal aircraftCount As Long, ByVal jobCount As Long, ByVal soldCount As Long, ByVal addCount As Long, ByVal updateCount As Long, ByVal noteCount As Long) As String
    ...
    html = html & TIR_DashMetricCell("SELLS", CStr(soldCount), "#7030A0")
    html = html & TIR_DashMetricCell("ADDS", CStr(addCount), "#008000")
...
Private Function TIR_DashAircraftCards(ByVal aircraftOrder As Object, ByVal aircraftDisplay As Object, ByVal aircraftJobs As Object, ByVal aircraftSold As Object, ByVal aircraftAdds As Object, ByVal aircraftUpdates As Object, ByVal aircraftCompletedList As Object, ByVal aircraftNewList As Object, ByVal aircraftUpdatedList As Object) As String
    ...
    html = "<div style='font-size:24pt;font-weight:bold;color:#0B2F6B;margin:18px 0 12px 0;text-align:center;letter-spacing:.5px;'>AIRCRAFT BRIEFING PANELS</div>"
    html = html & "<table align='center' cellpadding='0' cellspacing='0' style='border-collapse:separate;border-spacing:16px 18px;width:1320px;font-family:Calibri,Arial,sans-serif;font-size:12pt;margin:0 auto;'>"

    colCount = 0
    For Each ac In aircraftOrder.Keys
        If colCount = 0 Then html = html & "<tr>"
        html = html & "<td style='width:644px;vertical-align:top;padding:0;'>"
    ...
    If colCount = 1 Then html = html & "<td style='width:644px;'>&nbsp;</td></tr>"
    html = html & "</table>"
...
Private Function TIR_DashAircraftCard(ByVal ac As String, ByVal displayName As String, ByVal jobCount As Long, ByVal soldCount As Long, ByVal addCount As Long, ByVal updateCount As Long, ByVal completedHtml As String, ByVal newHtml As String, ByVal updatedHtml As String) As String
    ...
    parsedLocation = ""
    If InStr(1, displayName, " - ", vbTextCompare) > 0 Then
        displayTitle = Trim$(Left$(displayName, InStr(1, displayName, " - ", vbTextCompare) - 1))
        parsedLocation = Trim$(Mid$(displayName, InStr(1, displayName, " - ", vbTextCompare) + 3))
    End If
    If locationText = "" Then locationText = parsedLocation
    ...
    html = "<div style='border:1px solid #BFBFBF;border-left:9px solid " & accentColor & ";background:#FFFFFF;min-height:465px;'>"

    ' Header line 1: aircraft
    html = html & "<div style='background:#0B2F6B;color:#FFFFFF;padding:10px 14px 8px 14px;'>"
    html = html & "<div style='font-size:24pt;font-weight:bold;line-height:26pt;letter-spacing:.3px;'>" & TIR_HtmlEncode(UCase$(displayTitle)) & "</div>"
    If vhvzText <> "" Then html = html & "<div style='font-size:10.5pt;color:#D9EAF7;font-weight:bold;margin-top:2px;'>" & TIR_HtmlEncode(UCase$(vhvzText)) & "</div>"
    html = html & "</div>"

    ' Header line 2: location
    html = html & "<div style='background:#F7D774;color:#000000;padding:6px 14px;text-align:center;border-bottom:1px solid #C9A227;'>"
    If locationText <> "" Then
        html = html & "<span style='font-size:17pt;font-weight:bold;'>" & TIR_HtmlEncode(UCase$(locationText)) & "</span>"
    Else
        html = html & "<span style='font-size:17pt;font-weight:bold;'>LOCATION NOT SET</span>"
    End If
    html = html & "</div>"

    ' Header line 3: aircraft status before metrics
    If statusText <> "" Then
        html = html & "<div style='background:#FFF2F2;border-bottom:1px solid #E6B8B7;padding:7px 14px;text-align:center;'>"
        html = html & "<span style='font-size:10.5pt;color:#7F0000;font-weight:bold;'>AIRCRAFT STATUS:&nbsp;</span>"
        html = html & "<span style='font-size:15pt;color:#C00000;font-weight:bold;'>" & TIR_HtmlEncode(UCase$(statusText)) & "</span>"
        html = html & "</div>"
    Else
        html = html & "<div style='background:#F2F2F2;border-bottom:1px solid #D9D9D9;padding:7px 14px;text-align:center;'>"
        html = html & "<span style='font-size:10.5pt;color:#666666;font-weight:bold;'>AIRCRAFT STATUS:&nbsp;</span>"
        html = html & "<span style='font-size:13pt;color:#666666;font-weight:bold;'>NO NOTE ENTERED</span>"
        html = html & "</div>"
    End If

    html = html & "<div style='padding:12px 14px;'>"
    html = html & "<table cellpadding='0' cellspacing='0' style='border-collapse:collapse;width:100%;font-family:Calibri,Arial,sans-serif;margin-bottom:10px;'><tr>"
    html = html & TIR_DashKpiTile("TOTAL JOBS", CStr(jobCount), "#333333")
    html = html & TIR_DashKpiTile("SELLS", CStr(soldCount), "#7030A0")
    html = html & TIR_DashKpiTile("ADDS", CStr(addCount), "#008000")
    html = html & TIR_DashKpiTile("JOB STATUS CHANGES", CStr(updateCount), "#0057B8")
    html = html & "</tr></table>"

    If completedHtml = "" And newHtml = "" And updatedHtml = "" Then
        html = html & "<div style='font-size:13pt;color:#777777;border-top:1px solid #E6E6E6;padding-top:10px;font-weight:bold;'>NO SHIFT CHANGES</div>"
    Else
        html = html & "<div style='border-top:1px solid #E6E6E6;padding-top:8px;'>"
        If completedHtml <> "" Then html = html & TIR_DashPanelGroup("SELLS THIS SHIFT", completedHtml, "#7030A0")
        If newHtml <> "" Then html = html & TIR_DashPanelGroup("ADDS", newHtml, "#008000")
...
Private Function TIR_DashKpiTile(ByVal labelText As String, ByVal valueText As String, ByVal colorText As String) As String
    Dim html As String

    html = "<td style='width:25%;border-right:1px solid #D9D9D9;text-align:center;padding:6px 4px;'>"
    html = html & "<div style='font-size:24pt;line-height:24pt;font-weight:bold;color:" & colorText & ";'>" & TIR_HtmlEncode(valueText) & "</div>"
    html = html & "<div style='font-size:9.5pt;line-height:11pt;color:#333333;font-weight:bold;'>" & TIR_HtmlEncode(UCase$(labelText)) & "</div>"
    html = html & "</td>"

    TIR_DashKpiTile = html
End Function
...
Private Function TIR_DashPanelGroup(ByVal titleText As String, ByVal itemHtml As String, ByVal colorText As String) As String
    ...
    html = "<div style='margin:8px 0 6px 0;'>"
    html = html & "<div style='font-size:12pt;font-weight:bold;color:" & colorText & ";margin-bottom:3px;'>" & TIR_HtmlEncode(UCase$(titleText)) & "</div>"
    html = html & itemHtml
    html = html & "</div>"
...
Private Function TIR_DashPanelItem(ByVal jobNum As String, ByVal textValue As String, ByVal colorText As String) As String
    ...
    html = "<div style='margin:2px 0 3px 8px;color:" & colorText & ";line-height:16pt;font-size:12pt;'>"
...
Private Function TIR_DashDetailSection(ByVal aircraftOrder As Object, ByVal aircraftDisplay As Object, ByVal aircraftSold As Object, ByVal aircraftAdds As Object, ByVal aircraftUpdates As Object, ByVal aircraftCompletedList As Object, ByVal aircraftNewList As Object, ByVal aircraftUpdatedList As Object, ByVal aircraftCategories As Object, ByVal categoryJobs As Object) As String
    ...
    For Each ac In aircraftOrder.Keys
        ...
        changesHtml = ""
        If CStr(aircraftCompletedList(ac)) <> "" Then changesHtml = changesHtml & TIR_DashPanelGroup("SELLS THIS SHIFT", CStr(aircraftCompletedList(ac)), "#7030A0")
...
Private Function TIR_DashJobLine(ByVal jobNum As String, ByVal jobDesc As String, ByVal jobComment As String, ByVal colorText As String) As String
    ...
    html = "<table cellpadding='0' cellspacing='0' style='border-collapse:collapse;margin:3px 0 2px 16px;font-family:Calibri,Arial,sans-serif;font-size:12.5pt;line-height:16pt;color:" & colorText & ";'>"
    html = html & "<tr>"
    html = html & "<td style='width:106px;text-align:right;font-weight:bold;vertical-align:top;white-space:nowrap;'>" & TIR_HtmlEncode(UCase$(jobNum)) & "</td>"
    html = html & "<td style='padding-left:10px;vertical-align:top;'>" & TIR_HtmlEncode(UCase$(jobDesc)) & "</td>"
    html = html & "</tr>"
    ...
    If jobComment <> "" Then
        ...
        End If
        html = html & "<tr>"
        html = html & "<td style='width:106px;'>&nbsp;</td>"
        html = html & "<td style='padding-left:10px;padding-top:1px;padding-bottom:5px;color:" & commentColor & ";font-weight:" & commentWeight & ";font-size:12pt;line-height:15pt;'>"
...
Private Function TIR_BuildEmailHtml(ByVal ws As Worksheet, ByVal firstRow As Long, ByVal lastRow As Long) As String
    ...
    html = ""
    html = html & "<html><head><meta charset='utf-8'></head><body style='margin:0;padding:0;background-color:#FFFFFF;font-family:Calibri,Arial,sans-serif;font-size:11pt;color:#202020;'>"
    ...
    For r = firstRow To lastRow
        ...
        ElseIf TIR_IsLegendRow(bText, cText, dText) Or TIR_IsSummaryRow(bText, cText, dText) Then
            ' Print version skips the email summary/header rows.
            ' The PDF gets its own clean title from TIR_BuildPrintVersionHtml.
        ...
        ElseIf TIR_IsBulletRow(bText, cText, dText) Or Left(cText, 1) = ChrW(8226) Or Left(dText, 1) = ChrW(8226) Then
            ...
            If commentColor = "#000000" Then commentColor = TIR_GetCellColor(ws.Cells(r, "D"))
            If commentColor = "#000000" Then commentColor = "#0000CC"
            html = html & "<div style='margin-left:160px;margin-top:1px;margin-bottom:5px;color:" & commentColor & ";font-weight:bold;font-size:10.5pt;line-height:15pt;font-size:11.5pt;'>&bull;&nbsp;" & TIR_HtmlEncode(UCase$(jobText)) & "</div>"
...
Private Function TIR_CleanCellText(ByVal rawText As String) As String
    ...
    rawText = Replace(rawText, "â€¢", ChrW(8226))
    rawText = Replace(rawText, "â€˘", ChrW(8226))
