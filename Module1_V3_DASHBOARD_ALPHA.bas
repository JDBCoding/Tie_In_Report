...
End Sub
 ...
 ' ============================================================
' DASHBOARD EMAIL BUILDER v3.0 ALPHA
' Purpose:
...
Private Function TIR_BuildDashboardEmailHtml(ByVal ws As Worksheet, ByVal firstRow As Long, ByVal lastRow As Long) As String
    ...
    html = html & "<div style='width:1060px;font-family:Calibri,Arial,sans-serif;font-size:11pt;color:#222222;'>"
...
Private Function TIR_DashExecutiveSummary(ByVal aircraftCount As Long, ByVal jobCount As Long, ByVal soldCount As Long, ByVal addCount As Long, ByVal updateCount As Long, ByVal noteCount As Long) As String
    ...
    html = html & "<div style='font-size:20pt;font-weight:bold;letter-spacing:.5px;color:#1F1F1F;'>TIE-IN SHIFT TURNOVER DASHBOARD</div>"
    html = html & "<div style='font-size:9pt;color:#666666;margin-top:2px;'>WHAT CHANGED FIRST. FULL AIRCRAFT WORK LIST BELOW. GENERATED " & TIR_HtmlEncode(UCase$(reportTime)) & "</div>"
    html = html & "<table cellpadding='0' cellspacing='0' style='border-collapse:collapse;margin-top:12px;font-family:Calibri,Arial,sans-serif;font-size:11pt;'><tr>"
...
Private Function TIR_DashMetricCell(ByVal labelText As String, ByVal valueText As String, ByVal colorText As String) As String
    ...
    html = html & "<div style='font-size:20pt;font-weight:bold;color:" & colorText & ";line-height:22pt;'>" & TIR_HtmlEncode(valueText) & "</div>"
...
Private Function TIR_DashAircraftCards(ByVal aircraftOrder As Object, ByVal aircraftDisplay As Object, ByVal aircraftJobs As Object, ByVal aircraftSold As Object, ByVal aircraftAdds As Object, ByVal aircraftUpdates As Object, ByVal aircraftCompletedList As Object, ByVal aircraftNewList As Object, ByVal aircraftUpdatedList As Object) As String
    ...
    html = "<div style='font-size:16pt;font-weight:bold;color:#222222;margin:10px 0 8px 0;'>AIRCRAFT BRIEFING PANELS</div>"
    html = html & "<table cellpadding='0' cellspacing='0' style='border-collapse:separate;border-spacing:0 12px;width:1060px;font-family:Calibri,Arial,sans-serif;font-size:11pt;'>"

    colCount = 0
    For Each ac In aircraftOrder.Keys
        If colCount = 0 Then html = html & "<tr>"
        html = html & "<td style='width:520px;vertical-align:top;padding-right:14px;'>"
    ...
    If colCount = 1 Then html = html & "<td style='width:520px;'>&nbsp;</td></tr>"
    html = html & "</table>"
...
Private Function TIR_DashAircraftCard(ByVal ac As String, ByVal displayName As String, ByVal jobCount As Long, ByVal soldCount As Long, ByVal addCount As Long, ByVal updateCount As Long, ByVal completedHtml As String, ByVal newHtml As String, ByVal updatedHtml As String) As String
    ...
    html = "<div style='border:1px solid #BFBFBF;border-left:8px solid " & accentColor & ";background:#FFFFFF;min-height:310px;'>"
    html = html & "<div style='background:#FFF2CC;border-bottom:1px solid #D6B656;padding:10px 12px;'>"
    html = html & "<div style='font-size:18pt;font-weight:bold;color:#000000;line-height:21pt;'>" & TIR_HtmlEncode(UCase$(displayName)) & "</div>"
    If vhvzText <> "" Or locationText <> "" Then
        html = html & "<div style='font-size:10pt;color:#444444;margin-top:2px;'>"
    ...
    html = html & "<div style='padding:10px 12px;'>"
    html = html & "<table cellpadding='0' cellspacing='0' style='border-collapse:collapse;width:100%;font-family:Calibri,Arial,sans-serif;font-size:11pt;margin-bottom:8px;'>"
    ...
    html = html & TIR_DashCountRow("UPDATED COMMENTS", CStr(updateCount), "#0057B8")
    html = html & "</table>"

    If statusText <> "" Then
        html = html & "<div style='border-left:5px solid #C00000;background:#FFF2F2;padding:7px 9px;margin:8px 0 10px 0;'>"
        html = html & "<div style='font-size:8pt;color:#7F0000;font-weight:bold;'>AIRCRAFT STATUS</div>"
        html = html & "<div style='font-size:12pt;color:#C00000;font-weight:bold;'>" & TIR_HtmlEncode(UCase$(statusText)) & "</div>"
        html = html & "</div>"
    End If

    If completedHtml = "" And newHtml = "" And updatedHtml = "" Then
        html = html & "<div style='font-size:10.5pt;color:#777777;border-top:1px solid #E6E6E6;padding-top:8px;'>NO SHIFT CHANGES</div>"
    Else
        ...
        If updatedHtml <> "" Then html = html & TIR_DashPanelGroup("UPDATED COMMENTS", updatedHtml, "#0057B8")
        html = html & "</div>"
    End If
...
Private Function TIR_DashCountRow(ByVal labelText As String, ByVal valueText As String, ByVal colorText As String) As String
    ...
    html = "<tr>"
    html = html & "<td style='padding:2px 4px 2px 0;color:#555555;font-size:9pt;font-weight:bold;'>" & TIR_HtmlEncode(UCase$(labelText)) & "</td>"
    html = html & "<td style='padding:2px 0;text-align:right;color:" & colorText & ";font-size:14pt;font-weight:bold;width:44px;'>" & TIR_HtmlEncode(valueText) & "</td>"
    html = html & "</tr>"
...
Private Function TIR_DashPanelGroup(ByVal titleText As String, ByVal itemHtml As String, ByVal colorText As String) As String
    ...
    html = html & "<div style='font-size:9pt;font-weight:bold;color:" & colorText & ";margin-bottom:2px;'>" & TIR_HtmlEncode(UCase$(titleText)) & "</div>"
    html = html & itemHtml
    html = html & "</div>"
...
Private Function TIR_DashPanelItem(ByVal jobNum As String, ByVal textValue As String, ByVal colorText As String) As String
    ...
    html = "<div style='margin:2px 0 2px 8px;color:" & colorText & ";line-height:13pt;'>"
...
Private Function TIR_DashDetailSection(ByVal aircraftOrder As Object, ByVal aircraftDisplay As Object, ByVal aircraftSold As Object, ByVal aircraftAdds As Object, ByVal aircraftUpdates As Object, ByVal aircraftCompletedList As Object, ByVal aircraftNewList As Object, ByVal aircraftUpdatedList As Object, ByVal aircraftCategories As Object, ByVal categoryJobs As Object) As String
    ...
    html = "<div style='font-size:16pt;font-weight:bold;color:#222222;margin:20px 0 8px 0;border-top:3px solid #333333;padding-top:10px;'>FULL AIRCRAFT WORK LIST</div>"

    For Each ac In aircraftOrder.Keys
        html = html & "<div style='margin:12px 0 18px 0;border:1px solid #D9D9D9;background:#FFFFFF;'>"
        html = html & "<div style='background:#FFF2CC;padding:7px 10px;font-size:15pt;font-weight:bold;color:#000000;border-bottom:1px solid #D6B656;'>" & TIR_HtmlEncode(UCase$(CStr(aircraftDisplay(ac)))) & "</div>"
        ...
        If CStr(aircraftUpdatedList(ac)) <> "" Then changesHtml = changesHtml & TIR_DashPanelGroup("UPDATED COMMENTS", CStr(aircraftUpdatedList(ac)), "#0057B8")
...
Private Function TIR_DashJobLine(ByVal jobNum As String, ByVal jobDesc As String, ByVal jobComment As String, ByVal colorText As String) As String
    ...
    html = "<div style='margin:2px 0 1px 18px;color:" & colorText & ";line-height:14pt;'>"
    html = html & "<span style='display:inline-block;width:108px;font-weight:bold;text-align:right;'>" & TIR_HtmlEncode(UCase$(jobNum)) & "</span>"
    html = html & "<span style='display:inline-block;margin-left:10px;'>" & TIR_HtmlEncode(UCase$(jobDesc)) & "</span>"
    html = html & "</div>"

    If jobComment <> "" Then
        html = html & "<div style='margin:0 0 4px 150px;color:#0057B8;font-weight:bold;'>"
