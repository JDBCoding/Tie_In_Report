...
Sub OpenEmailTieIn()
    ...
    ' Find the last row in columns B, C, and D
    lastRow = Application.WorksheetFunction.Max(ws.Cells(ws.Rows.count, "B").End(xlUp).Row, _
                                                 ws.Cells(ws.Rows.count, "C").End(xlUp).Row, _
                                                 ws.Cells(ws.Rows.count, "D").End(xlUp).Row)

    ' Set the range to copy from columns B, C, and D starting from row 4
    Set ExcelRange = ws.Range("B4:D" & lastRow) ' Adjust starting row as needed
    ...
    ' Create a new email
    With outlookMail
        .To = "DL-BDSKC46FieldTiein@exchange.boeing.com" ' Set the recipient's email address
        .Subject = "BDS Field Tie in" ' Set the subject line
...
' Option Explicit  ' disabled: must be at top of module
